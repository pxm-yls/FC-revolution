using FCRevolution.Emulation.Abstractions;
using FC_Revolution.UI.Adapters.LegacyTimeline;

namespace FC_Revolution.UI.Tests;

[Collection("Avalonia")]
public sealed class GameWindowPreviewNodeFactoryTests
{
    [Fact]
    public void Create_MapsRecordFields_AndBuildsBitmapWithRequestedSize()
    {
        var snapshot = new CoreTimelineSnapshot
        {
            Frame = 321,
            TimestampSeconds = 5.35,
            Thumbnail = new uint[64 * 60],
            State = new CoreStateBlob
            {
                Format = "test/snapshot",
                Data = []
            }
        };
        var entry = new TimelinePreviewEntry(
            SnapshotId: Guid.NewGuid(),
            BranchId: Guid.NewGuid(),
            Frame: 321,
            TimestampSeconds: 5.35,
            CreatedAtUtc: DateTime.UtcNow,
            Name: "自定义节点标题",
            Snapshot: snapshot);

        var node = GameWindowPreviewNodeFactory.Create(entry, previewWidth: 256, previewHeight: 240);

        Assert.Equal(entry.SnapshotId, node.Id);
        Assert.Equal(entry.Frame, node.Frame);
        Assert.Equal(entry.TimestampSeconds, node.TimestampSeconds);
        Assert.Equal("自定义节点标题", node.Title);
        Assert.NotNull(node.Bitmap);
        Assert.Equal(256, node.Bitmap!.PixelSize.Width);
        Assert.Equal(240, node.Bitmap.PixelSize.Height);
    }

    [Fact]
    public void Create_UsesFallbackTitle_WhenRecordNameMissing()
    {
        var entry = new TimelinePreviewEntry(
            SnapshotId: Guid.NewGuid(),
            BranchId: Guid.NewGuid(),
            Frame: 42,
            TimestampSeconds: 0.7,
            CreatedAtUtc: DateTime.UtcNow,
            Name: null,
            Snapshot: new CoreTimelineSnapshot
            {
                Frame = 42,
                TimestampSeconds = 0.7,
                Thumbnail = new uint[64 * 60],
                State = new CoreStateBlob
                {
                    Format = "test/snapshot",
                    Data = []
                }
            });

        var node = GameWindowPreviewNodeFactory.Create(entry, previewWidth: 128, previewHeight: 96);

        Assert.Equal("画面节点 42", node.Title);
    }
}
