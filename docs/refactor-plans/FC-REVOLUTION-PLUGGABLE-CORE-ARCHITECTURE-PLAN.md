...
19. 图形化 [FC-Revolution.Core.Workbench.csproj](/Users/pxm/Desktop/Cs/FC/FC-Revolution/tools/FC-Revolution.Core.Workbench/FC-Revolution.Core.Workbench.csproj) 已落地为 MVP：它通过共享 runtime 暴露 resource root、probe directories、`*.fcrcore.zip` 临时工作区、catalog 刷新、smoke check 与单帧 preview capture，不再需要从主程序复制第二套 loader/package/probe-path 逻辑；对应 focused test 见 [CoreWorkbenchViewModelTests.cs](/Users/pxm/Desktop/Cs/FC/FC-Revolution/tests/FC-Revolution.UI.Tests/Tools/CoreWorkbenchViewModelTests.cs)。
...
5. `Core Workbench` 与 `native-cabi` 都已经进入 MVP 阶段，但 `Core Workbench` 目前只补到了 catalog/smoke/单帧 preview capture，还未接入 export/debug 等更完整的核心开发工作流；native loader 也仍缺 richer input/video capability 映射与外部核心仓库样板。
...
3. 当前未完成的部分是 export/debug workflow 与外部核心仓库试点；preview capture 已通过共享 `CorePreviewFrameCaptureService` 接入。
...
3. 当前剩余的是 richer capability bridge、跨平台/跨仓库样板与更完整的 native ABI 文档化。
