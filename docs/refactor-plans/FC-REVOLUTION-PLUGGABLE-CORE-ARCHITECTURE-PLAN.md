...
7. `SessionRemoteControlService` 已把 `claim/release/heartbeat` 的 `portId -> player` 解析收口到单点 helper，并把 `button/input` 新写路径继续下沉到 `IGameSessionService -> GameWindowViewModel.SetRemoteInputState(...)` 通用入口；当前当 core `systemId = nes` 时，`x/y/turboA/turboB` 仍在 UI 会话层走 alias 桥接、`l1/r1/l2/r2/l3/r3` 在 NES core 下安全 no-op；当非 NES core 在 `InputSchema` 中声明 action 时，对应 action 已可原样直通 runtime，不再由 Application 层强制回映射到 `NesButton`。
8. `ClaimControlRequest`、`ReleaseControlRequest`、`RefreshHeartbeatRequest`、`ButtonStateRequest` 与 backend WebSocket/HTTP/WebPad 入口已进入“`portId` 优先 + legacy `player`/`NesButtonDto` 兼容”的双栈过渡期；contracts 层本轮已补上 `RemoteControlRequestCompatibility` 共享转换器，`BackendContractClient`、HTTP `/buttons` 兼容入口与 WebSocket `button` 事件开始统一在边界层把 `ActionId` 转成 `SetInputStateRequest`，generic button 写路径已优先走通用 `input` 协议；同时 `ArcadeRuntimeContractAdapter.SetButtonStateAsync(...)` 也已切到 generic-first（`ActionId` 存在时优先转 `SetInputStateRequest`，仅纯 legacy payload 才回落到 `SetButtonState` 兼容壳）；WebPad 新写路径已经默认改走通用 `input + inputs[]` 协议，控制区也已补上 `A/B/X/Y` 布局、`X -> Turbo A` / `Y -> Turbo B` 默认前端连发行为，以及 `L1/R1/L2/R2/L3/R3` 预留按钮位。
...
11. `MainWindowActiveInputRuntimeController` 已把 `MainWindowViewModel.InputAndShortcuts` 中的 `_pressedKeys`、turbo pulse、desired mask 计算、transition 生成与 `ICoreInputStateWriter` 写回下沉到独立运行时 controller；此前已补上 `MainWindowActiveInputWorkflowController` 与 `MainWindowInputKeyboardWorkflowController`，把 active input ROM 路径解析、refresh/apply 决策、turbo pulse 编排、legacy mirror 生成，以及 `OnKeyDown/OnKeyUp/ShouldHandleKey` 的键盘工作流/动作分发继续从 partial 中抽离；本轮再补上 `MainWindowInputBindingWorkflowController`，并让 `MainWindowActiveInputWorkflowController` 增加一站式 `RefreshActiveInputState(...)` 编排，把 global/ROM input binding view-state apply、ROM binding refresh，以及 effective key map / extra binding / handled key 组装继续从 partial 中收走。主窗口输入 partial 已退回到输入绑定命令、active input apply 与最终 legacy mirror 同步薄壳；按 `FC-Revolution.UI / LAN / 预览治理任务清单` 的口径，`1.5` 已可标记完成。
...
2. 远控 generic button 写路径虽已开始在 `BackendContractClient`、HTTP `/buttons` 兼容入口与 WebSocket `button` 边界层优先走 `input` 协议，但 contracts 仍保留 `p1/p2` 端口命名与 `NesButtonDto` compatibility shell，尚未升级为真正的系统无关动作协议。
3. 主窗口虽然已经有默认核心选择、来源目录管理/重载、本地 DLL 导入/卸载与来源摘要 UI，但还缺远程下载 / 更新 / 版本管理闭环以及更完整的 manifest 展示，当前仍停留在基础挂载能力。
