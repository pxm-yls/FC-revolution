...
2. timeline 的纯存储路径层、replay log 文件格式层与 snapshot base-frame 读取层已经通用化，但 UI 与 `FC-Revolution.FC.LegacyAdapters` 之间仍保留 `LegacyTimelineRepositoryAdapter` / `LegacyTimelineManifestHandle` / `LegacyTimelinePreviewEntry` 这类 FC/NES 专用 bridge，`TimelineVideoExporter.ExportMp4(...)` 也还直接依赖 `NesReplayInterop` 去执行 NES 专用 replay 渲染。
