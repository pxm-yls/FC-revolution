...
2. timeline 的纯存储路径层已经通用化，但 UI 与 `FC-Revolution.FC.LegacyAdapters` 之间仍保留 `LegacyTimelineRepositoryAdapter` / `LegacyTimelineManifestHandle` / `LegacyTimelinePreviewEntry` 这类 FC/NES 专用 bridge，`TimelineVideoExporter` / `TimelineInputLogWriter` 也还直接依赖 `NesReplayInterop` / `NesReplayLogWriter`。
