...
2. 远控 generic button 写路径虽已在 `BackendContractClient`、HTTP `/buttons` 兼容入口与 WebSocket `button` 边界层优先走 `input` 协议，backend WebSocket/lease internals 也已切到 `portId-first`，但 contracts 仍保留 `RemoteControlPorts` / `p1/p2` 端口命名以及 `SetButtonState` / `/buttons` compatibility shell，尚未升级为真正的系统无关动作协议。
