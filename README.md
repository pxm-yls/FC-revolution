...
当前 FC / NES 核心目录位于 `src/FC-Revolution.Core/FC-Revolution.Core.FC`，已经集中包含核心 engine 与 managed adapter，主要模块包括：
...
其中 `src/FC-Revolution.Core/FC-Revolution.Core.FC/Managed` 下的托管核心模块 `FC-Revolution.Core.Nes.Managed` 已通过统一宿主接口暴露以下能力：
...
- 默认核心已经改成配置驱动；如果未配置则回退到首个已安装核心，没有核心时允许零核心启动
- 架构上已为未来 `native-cabi` 和多系统核心预留入口

这部分设计详见 `docs/refactor-plans/FC-REVOLUTION-PLUGGABLE-CORE-ARCHITECTURE-PLAN.md`。
...
这部分设计详见 `docs/feature-overviews/RENDERING_PLATFORM_GUIDE.md` 与 `docs/feature-overviews/MACOS_DEVELOPMENT_GUIDE.md`。
...
- `FC-Revolution.Core.FC`
  - 当前 FC / NES 核心目录，内部同时包含 engine 与 managed adapter
...
├── docs/                          文档索引与分类归档
│   ├── README.md                 docs 索引
│   ├── refactor-plans/          重构方案与任务清单
│   ├── whats-new/               新内容与阶段性产出
│   ├── feature-overviews/       功能介绍与平台路线
│   └── feature-guides/          新建功能说明与执行手册
├── src/
...
1. `README.md`
2. `docs/README.md`
3. `docs/refactor-plans/FC-REVOLUTION-PLUGGABLE-CORE-ARCHITECTURE-PLAN.md`
4. `docs/feature-overviews/RENDERING_PLATFORM_GUIDE.md`
5. `docs/feature-overviews/MACOS_DEVELOPMENT_GUIDE.md`
6. `docs/feature-guides/FC-REVOLUTION-NEW-EMULATOR-CORE-INTEGRATION-GUIDE.md`
7. `src/FC-Revolution.UI`
8. `src/FC-Revolution.Core/FC-Revolution.Core.FC`
